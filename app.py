...
#st.title("🌿 Allergy Flare-Up & Trigger Dashboard")
st.write(" ")
st.write(" ")
...
DATA_FILE = "allersafe_entries.csv"
def log_user_entry(user_data, prob):
    entry = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "season": user_data['Season'],
        "temperature": user_data['Temperature'],
        "humidity": user_data['Humidity'],
        "dust_level": user_data['Dust Level'],
        "smoke_level": user_data['Smoke Level'],
        "outdoor_time_min": user_data['Outdoor Time (minutes)'],
        "medication_used": user_data['Medication Used'],
        "symptom_score": user_data['Current Symptom Score'],
        "predicted_risk": prob
    }
...
st.write(" ")
st.markdown("<h3 style='text-align: center; color:green;'> Thank you for trusting AllerSafe 🌿</h3>",
            unsafe_allow_html=True)
